...
class iplb(object):
    ...
    def __init__(self, connection, service_ip, servers=[]):
        ...
        # Initialize total connection for each server
        self.total_connection = {}
        for ip in self.servers:
            self.total_connection[ip] = 0
        self.log.debug("Initialize total connection\n %s", self.total_connection)
        ...
        if not tcpp:
            ...
            if udpp and ipp:
                if ipp.dstip == self.service_ip and ipp.srcip in self.live_servers:
                    self.total_connection[ipp.srcip] = float(pickle.loads(udpp.next))
